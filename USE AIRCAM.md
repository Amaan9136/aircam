...
Do **not** use the `/desktop` dashboard as your OBS Browser Source — it
captures the full page (title, Connect button, room code box), not just
the video. Instead, use the clean, video-only route:

```
https://aircam.onrender.com/desktop/clean/<room>
```

Replace `<room>` with the code shown on `/desktop`. This page shows only
the camera feed on a black background and connects automatically, so OBS
captures nothing but the video.
